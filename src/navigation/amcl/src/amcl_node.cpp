...
class AmclNode
{
  ...
  private:
    ...
    static pf_vector_t gaussianPoseGenerator(void* arg); // modified
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  if(lasers_update_[laser_index])
  {
    ...
    for(int i=0; i<set_tmp->sample_count; i++)
    {
      ...
      // if(i%100 == 0) ROS_INFO("weight : %f", set_tmp->samples[i].weight);
    }

    avg_weight = sum_weight/set_tmp->sample_count;
    ROS_INFO("avg_weight: %f\n", avg_weight);
    ROS_INFO("h_wieght: %f\n", h_weight);
    ROS_INFO("l_weight: %f\n", l_weight);
    ROS_INFO("w_fast : %f, w_slow : %f", pf_->w_fast, pf_->w_slow);
    ...
    pf_->random_pose_fn = (pf_init_model_fn_t)AmclNode::gaussianPoseGenerator; // 현재 로봇 위치를 중심으로 가우시안 분포를 따르는 랜덤 파티클 생성
    //************************** modified!!: 랜덤하게 파티클이 생성될 확률을 고정 수치 할당
    pf_->w_fast = 9.5;
    pf_->w_slow = 10.0;
    //***************************
...
// compute gaussian random function
double gaussianRandom(double average, double stdev)
{
  double v1, v2, s, temp;

  do{
    v1 = 2 * ((double) rand() / RAND_MAX) - 1; // from -1.0 to 1.0
    v2 = 2 * ((double) rand() / RAND_MAX) - 1; // from -1.0 to 1.0
    s = v1 * v1 + v2 * v2;
  }while(s >= 1|| s == 0);
...
// 가우시안 랜덤 함수를 통하여 계산된 결과를 특정 파티클의 pose에 추가하여 리턴
pf_vector_t
AmclNode::gaussianPoseGenerator(void* arg)
{
  double location_error_x = 0.75;
  double location_error_y = 0.75;
  ...
  for(;;)
  {
    p.v[0] = temp_x + gaussianRandom(0, location_error_x);
    p.v[1] = temp_y + gaussianRandom(0, location_error_y);
